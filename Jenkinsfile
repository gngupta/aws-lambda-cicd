...
podTemplate(label: 'jenkins-pipeline', containers: [
	...
	node('jenkins-pipeline') {
		...
		stage('Deploy') {
			container('aws') {
				withAWS(credentials: config.lambda.credentialId) {
					sh "aws lambda update-function-code --function-name ${config.lambda.name} --s3-bucket ${config.lambda.s3Bucket} --zip-file fileb://./${env.BRANCH_NAME}/${artifactName}.jar --s3-key ${artifactName}.jar --region ${config.lambda.region}"
				}
			}
		}
